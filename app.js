...
const sound = {
  ...
  flyAway() {
    ...
    mk("sawtooth", 160, 55, 0.32, 0.95, 0);
    mk("square", 220, 70, 0.12, 0.7, 0.02);
    try {
      ...
      ng.gain.exponentialRampToValueAtTime(0.26, now + 0.03);
...
function drawFrame() {
  ...
  if (flight.flying || flight.crashed) {
    ...
    if (flight.crashed) {
      // Accelerate hard off-screen
      flight.flyAwayT = (flight.flyAwayT || 0) + 0.055 + (flight.flyAwayT || 0) * 0.12;
      t = Math.min(2.2, t + flight.flyAwayT);
    }
    ...
    if (px < canvas.w + 100 && py > -100) {
      const shrink = flight.crashed ? Math.max(0.35, 1 - (flight.flyAwayT || 0) * 0.35) : 1;
