...
const sound = {
  ...
  flyAway() {
    ...
    const mk = (type, f0, f1, g0, dur, delay) => {
      ...
      osc.frequency.exponentialRampToValueAtTime(Math.max(40, f1), now + delay + dur);
      filt.type = "lowpass";
      filt.frequency.setValueAtTime(2200, now + delay);
      filt.frequency.exponentialRampToValueAtTime(500, now + delay + dur);
      gain.gain.setValueAtTime(0.0001, now + delay);
      gain.gain.exponentialRampToValueAtTime(g0, now + delay + 0.04);
    ...
    mk("sawtooth", 160, 55, 0.42, 0.95, 0);
    mk("square", 220, 70, 0.18, 0.7, 0.02);
    try {
      const n = this.ctx.createBufferSource();
      const buf = this.ctx.createBuffer(1, Math.floor(this.ctx.sampleRate * 1.1), this.ctx.sampleRate);
...
function pathPoint(t) {
  // Start at bottom (parked); climb with y ∝ t^1.25
  t = Math.max(0, Math.min(1, t));
  const x = 0.06 + 0.84 * t;
  const y = 0.90 - 0.68 * Math.pow(t, 1.25);
...
function pathTangent(t) {
  ...
  const dy = t <= 0 ? 0 : -0.68 * 1.25 * Math.pow(t, 0.25);
...
function drawFrame() {
  ...
  if (flight.flying || flight.crashed) {
    ...
    if (flight.crashed) {
      // Exit fast enough to read, but eye can still follow
      flight.flyAwayT = (flight.flyAwayT || 0) + 0.028 + (flight.flyAwayT || 0) * 0.06;
      t = Math.min(2.0, t + flight.flyAwayT);
    }
    ...
    if (t > 1) {
      const extra = t - 1;
      px = pt.x + extra * canvas.w * 1.1;
      py = pt.y - extra * canvas.h * 0.95;
    }
    // Hide once fully off stage
    if (px > canvas.w + 80 || py < -80) {
      /* vanished */
    }
    const angle = planeAngleFromProgress(Math.min(1, flight.progress + (flight.flyAwayT || 0))) + (flight.crashed ? -0.35 : 0);
    flight.planeAngle = angle;
    if (px < canvas.w + 100 && py > -100) {
      const shrink = flight.crashed ? Math.max(0.4, 1 - (flight.flyAwayT || 0) * 0.18) : 1;
      drawPlane(ctx, px, py, angle, PLANE_SCALE * shrink);
    }
  } else {
