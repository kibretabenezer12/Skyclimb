...
function startSmoothCountdown(secondsLeft) {
  stopSmoothCountdown();
  const sec = Math.max(0.2, Number(secondsLeft) || 7);
  state._waitEndAt = performance.now() + sec * 1000;
  state._waitStartAt = performance.now();
  ...
  const paint = () => {
    ...
    const frac = Math.max(0, Math.min(1, left / (state._waitTotal || sec)));
...
function pathPoint(t) {
  ...
  const x = 0.08 + 0.82 * t;
  // Climb into upper half, then ease slightly downward past mid so plane can sit mid-frame
  let yn;
  if (t < 0.5) {
    yn = 0.88 - 0.50 * Math.pow(t / 0.5, 1.35); // 0.88 → ~0.38
  } else {
    const u = (t - 0.5) / 0.5;
    yn = 0.38 + u * 0.12; // drift slightly lower after middle ~0.38 → 0.50
  }
  return { x: x * canvas.w, y: yn * canvas.h };
}
...
function drawPlane(ctx, x, y, angle, scale) {
  ...
  // Soft red glow (HD saturated)
  ctx.shadowColor = "rgba(255, 40, 40, 0.85)";
  ctx.shadowBlur = 14;

  // Fuselage — bright saturated red
  const body = ctx.createLinearGradient(-16, 0, 20, 0);
  body.addColorStop(0, "#c01010");
  body.addColorStop(0.35, "#ff1a1a");
  body.addColorStop(0.7, "#ff2d2d");
  body.addColorStop(1, "#ff5555");
  ctx.fillStyle = body;
  ctx.beginPath();
  ctx.moveTo(22, 0);
  ...
  ctx.lineTo(-16, 0);
  ctx.lineTo(-8, 4);
  ctx.quadraticCurveTo(12, 6, 22, 0);
  ctx.closePath();
  ctx.fill();

  // Wings
  ctx.fillStyle = "#e00808";
  ctx.beginPath();
  ctx.moveTo(4, 0);
  ctx.lineTo(-2, -14);
  ctx.lineTo(-7, -13);
  ...
  ctx.beginPath();
  ctx.ellipse(11, -1.2, 4, 2.4, 0, 0, Math.PI * 2);
  ctx.fill();

  ctx.restore();
}

/** Nose angle from path progress: mild early, after mid-sky settle ~25–30° (not steep to sky). */
