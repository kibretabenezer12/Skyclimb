...
function multiplierAt(tSec) {
  ...
  return Math.round((1.0 + Math.pow(tSec / 8.0, 1.35)) * 100) / 100;
}

function timeToReachMult(target) {
  if (target <= 1.0) return 0;
  return 8.0 * Math.pow(target - 1.0, 1 / 1.35);
}
...
function setFlightProgress(mult) {
  ...
  // Slightly higher coefficient so the plane still moves at a good pace
  // while the number climbs very slowly (divisor 8.0).
  const p = 1 - Math.exp(-(mult - 1) * 0.32);
