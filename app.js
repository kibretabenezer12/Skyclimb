...
function drawPlane(ctx, x, y, angle, scale) {
  ...
  ctx.translate(x, y);
  ctx.rotate(angle);
  // Sprite faces right; slight nose-up already in art
  const s = scale * 1.15;
  if (planeSprite.ready && planeSprite.img) {
    const img = planeSprite.img;
    const w = 56 * s;
    const h = (img.height / img.width) * w;
    ctx.shadowColor = "rgba(255, 30, 30, 0.65)";
    ctx.shadowBlur = 12;
    // Image points right; center on body
    ctx.drawImage(img, -w * 0.45, -h * 0.55, w, h);
    ctx.shadowBlur = 0;
  } else {
    // Fallback silhouette if image not loaded yet
