...
const sound = {
  ...
  flyAway() {
    ...
    // Loud overhead pass — whoosh by your head
    const osc = this.ctx.createOscillator();
    ...
    osc.frequency.exponentialRampToValueAtTime(70, now + 1.15);
    filt.type = "lowpass";
    filt.frequency.setValueAtTime(900, now);
    filt.frequency.exponentialRampToValueAtTime(2400, now + 0.3);
    filt.frequency.exponentialRampToValueAtTime(400, now + 1.1);
    ...
    try {
      ...
      nf.frequency.exponentialRampToValueAtTime(1800, now + 0.28);
      nf.frequency.exponentialRampToValueAtTime(300, now + 1.05);
      nf.Q.value = 0.7;
      ng.gain.setValueAtTime(0.0001, now);
      ng.gain.exponentialRampToValueAtTime(0.22, now + 0.05);
      ng.gain.exponentialRampToValueAtTime(0.0001, now + 1.15);
...
function pathPoint(t) {
  // Upper climb: y ∝ t^1.25 (flatter than 1.5)
  t = Math.max(0, Math.min(1, t));
  const x = 0.06 + 0.84 * t;
  const y = 0.82 - 0.58 * Math.pow(t, 1.25);
...
function pathTangent(t) {
  ...
  const dy = t <= 0 ? 0 : -0.58 * 1.25 * Math.pow(t, 0.25);
...
function setFlightProgress(mult) {
  // Target progress from mult; smooth lerp kills takeoff jump
  const target = Math.min(0.98, 1 - Math.exp(-(mult - 1) * 0.38 * 2.5));
  const cur = flight.progress || 0;
  // Ease toward target (stronger early so takeoff is smooth, not a snap)
  const blend = cur < 0.02 ? 0.12 : 0.28;
  flight.progress = cur + (target - cur) * blend;
  if (Math.abs(target - flight.progress) < 0.001) flight.progress = target;
