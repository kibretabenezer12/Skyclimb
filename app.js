...
function drawFrame() {
  ...
  if (flight.flying || flight.crashed) {
    ...
    if (flight.crashed) {
      // Slow, readable exit — about 1.5–2s to leave the stage
      flight.flyAwayT = (flight.flyAwayT || 0) + 0.0055 + (flight.flyAwayT || 0) * 0.012;
      t = Math.min(1.75, flight.progress + flight.flyAwayT);
    }
    ...
    if (t > 1) {
      const extra = t - 1;
      px = pt.x + extra * canvas.w * 0.45;
      py = pt.y - extra * canvas.h * 0.35;
    }
    ...
    if (px < canvas.w + 100 && py > -100) {
      const shrink = flight.crashed ? Math.max(0.6, 1 - (flight.flyAwayT || 0) * 0.05) : 1;
